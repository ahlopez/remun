...
import static com.transmi.remun.service.util.FrontConst.PAGE_USERS;
...
@PageTitle(FrontConst.TITLE_USERS)
@Secured(Role.ADMIN)
public class UsersView extends AbstractRemunCrudView<User>
{
  ...
  @Override
  protected String getBasePage() { return PAGE_USERS; }
  ...
  private static BinderCrudEditor<User> createForm(PasswordEncoder passwordEncoder) {
    ...
    binder.forField(password)
        .withValidator(pass-> pass.matches("^(|(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{6,})$"),
            "necesita 6 or más caracteres, mezcla de digitos, letras minúsculas y mayúsculas"
        )
        .bind(
