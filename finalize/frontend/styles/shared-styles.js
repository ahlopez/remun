...
$_documentContainer.innerHTML = `<dom-module id="remun-app-layout-theme" theme-for="vaadin-app-layout">
  <template>
    <style>
      :host {
...
<dom-module id="remun-login-theme" theme-for="vaadin-login-overlay-wrapper">
  <template>
    <style>
      [part="brand"] {
...
<dom-module id="remun-crud-theme" theme-for="vaadin-crud">
  <template>
    <style>
      :host {
        height: 100%;
...
<!-- Themes for dialogs -->
<dom-module id="remun-dialog-overlay-theme" theme-for="vaadin-dialog-overlay">
  <template>
    <style>
      :host([theme~="contracts"]) {
        padding: var(--lumo-space-xs);
      }

      :host([theme~="contracts"]) [part="overlay"] {
        display: flex;
      ...
      :host([theme~="contracts"]) [part="content"],
      :host([theme~="contracts"]) [part="overlay"] {
        max-width: 65em;
        width: 100%;
      }
...
<dom-module id="remun-text-field-theme" theme-for="vaadin-text-field">
  <template>
    <style>
      :host([status="vigente"]) [part~="input-field"] {
        color: var(--lumo-primary-color);
        background: var(--lumo-primary-color-10pct);
      }

      :host([status="cerrado"]) [part~="input-field"] {
...
<dom-module id="remun-grid-theme" theme-for="vaadin-grid">
  <template>
    <style>
      :host {
        width: 100%;
      ...
      /* a special grid theme for the remun storefront view */
      :host([theme~="contracts"]) {
        background: transparent;
      }

      :host([theme~="contracts"]) [part~="cell"]:not(:empty):not([details-cell]) {
        padding: 0;
      }

      :host([theme~="contracts"]) [part~="row"][selected] [part~="cell"] {
        background: transparent !important;
      }

      :host([theme~="contracts"]) [part~="body-cell"] {
        background: transparent;
      }

      @media (max-width: 600px) {
        :host([theme~="contracts"]) [part~="cell"] ::slotted(vaadin-grid-cell-content) {
          padding: 0 !important;
        }
      }
    </style>
